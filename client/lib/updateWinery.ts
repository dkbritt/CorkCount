...
export async function updateExistingWineryNames(): Promise<{ success: boolean; updated: number; error?: string }> {
  try {
    ...
    const itemsToUpdate = inventory.filter((item: any) => item.winery === 'CorkCount Winery');
    ...
    for (const item of itemsToUpdate) {
      try {
        ...
        if (updateResponse.ok && updateResult.success) {
          ...
          errors.push(`Failed to update item ${item.id}: ${updateResult.error}`);
        }
      } catch (err) {
        errors.push(`Error updating item ${item.id}: ${err instanceof Error ? err.message : 'Unknown error'}`);
      }
    }
